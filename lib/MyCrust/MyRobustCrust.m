...
tic
[tbound,~]=Marking(p,tetr,tetr2t,t2tetr,cc,r,nshield);%Flagging tetrahedrons as inside or outside
...
%Gets conectivity relantionships among tetrahedrons

numt = size(tetr,1);
vect = 1:numt;
t = [tetr(:,[1,2,3]); tetr(:,[2,3,4]); tetr(:,[1,3,4]);tetr(:,[1,2,4])];%triangles not unique
[t,j,j] = unique(sort(t,2),'rows');%triangles
t2tetr = [j(vect), j(vect+numt), j(vect+2*numt),j(vect+3*numt)];%each tetrahedron has 4 triangles
...
numtetr=size(tetr,1);
nt=size(tetr2t,1);
% deleted=true(numtetr,1);%deleted tetrahedrons
% checked=false(numtetr,1);%checked tetrahedrons
onfront=false(nt,1);%tetrahedrons that need to be checked
% countchecked=0;%counter of checked tetrahedrons
...
%vectorized
deleted=any(tetr>np,2);%deleted tetrahedrons
checked=deleted;%checked tetrahedrons
onfront(t2tetr(checked,:))=true;
countchecked=sum(checked);%counter of checked tetrahedrons
...
%intersection factor
%it is computed from radius of the tetrahedrons circumscribed sphere
...
%         Now we scan all tetrahedrons. When one is scanned puts on front is
%         neighbor. This means that now  the neighobor can be checked too.
%         At the begining only tetrahedrons with shield points are on front,
%         because we are sure the are out. Tetraedron with high
...
%         when a tetrahedron is wrong marked.
%
ids=1:nt;
queue=ids(onfront);
nt=length(queue);
while countchecked<numtetr && level<MAXLEVEL
    ...
    for i=1:nt%loop trough triangles <-----better is check only unchecked
        ...
        tetr1=tetr2t(id,1);tetr2=tetr2t(id,2);%tetrahedrons linked to triangle under analysis
        if  tetr2==0 %do not check boundary triangles
            onfront(id)=false;
            continue

        elseif (checked(tetr1) && checked(tetr2)) %tetrahedrons are already checked
            onfront(id)=false;
            continue
...
% fprintf('%4.4f %% of tetrahedrons were checked\n',numchecked*100)
...
%shield points are very good in detectinf outside tetrahedrons. Unfortunatly
...
tbound=sum(tbound,2)==1;%bounary triangles only have one tetrahedron
...
%unvectorized
% for i=1:nt
%     if tetr2t(i,2)>0 %jump boundary tetrahedron
%         distcc=sum((cc(tetr2t(i,1),:)-cc(tetr2t(i,2),:)).^2,2);%distance between circumcenters
%         %intersection factor
%         Ifact(i)=(-distcc+r(tetr2t(i,1))^2+r(tetr2t(i,2))^2)/(2*r(tetr2t(i,1))*r(tetr2t(i,2)));
%     end
...
%Intilize the front
         ...
         [~,t1]=max( (p(t(:,1),3)+p(t(:,2),3)+p(t(:,3),3))/3);
